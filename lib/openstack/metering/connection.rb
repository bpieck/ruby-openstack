module OpenStack
  module Metering
    ...
    class Connection
      ...
      def meters(options = {})
        path = OpenStack.get_query_params(options, [:q], "#{@connection.service_path}/meters")
      ...
      def bandwidth(options = {})
        path = OpenStack.get_query_params(options, [:q, :limit], "#{@connection.service_path}/meters/bandwidth")
        response = @connection.csreq('GET', @connection.service_host, path, @connection.service_port, @connection.service_scheme)
        OpenStack::Exception.raise_exception(response) unless response.code.match(/^20.$/)
