...
#
      ...
      def simple_tenant_usage(start_time=(Time.now - 3600), end_time=Time.now, tenant_id=nil)
        ...
        response_hash = OpenStack.symbolize_keys(JSON.parse(response.body))
        if tenant_id
          response_hash[:tenant_usage]
        else
...
#list all float ips associated with tennant or account
      ...
      # Not working
      # Nova does not support deletion via API
      def delete_floating_ips_bulk(opts)
        raise ArgumentError, 'Should not be empty' if opts.empty?
        check_extension 'os-floating-ips-bulk'
        response = @connection.req('POST', '/os-floating-ips-bulk/delete', {data: JSON.generate(opts)})
